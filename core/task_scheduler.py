...
class SchedulerMixin:
    ...
    async def _schedule_next_chat_and_save(
        ...
        if scheduled_job_payload is not None:
            nsid = scheduled_job_payload["normalized_session_id"]
            sid = scheduled_job_payload["session_id"]
            rd = scheduled_job_payload["run_date"]
        ...
        if scheduled_job_payload is not None:
            try:
                self.scheduler.add_job(
                    ...
                    args=[scheduled_job_payload["session_id"]],
                    id=scheduled_job_payload["session_id"],
                    replace_existing=True,
                    misfire_grace_time=60,
                )
                logger.info(
                    f"[主动消息] {self._get_session_log_str(scheduled_job_payload['session_id'], scheduled_job_payload['current_config'])} 满足条件，自动触发任务已创建喵！执行时间 (非持久化): {scheduled_job_payload['run_date'].strftime('%Y-%m-%d %H:%M:%S')} 喵"
                )
            except Exception as e:
                logger.error(f"[主动消息] 自动触发任务添加到调度器失败喵: {e}")
    ...
    def _cleanup_expired_session_states(self, current_time: float) -> None:
        ...
        # 清理 first_message_logged 中已不存在的会话
        first_msg_logged = getattr(self, "first_message_logged", None)
        if first_msg_logged and isinstance(first_msg_logged, set):
            active_sessions = set(self.session_data.keys())
            stale = first_msg_logged - active_sessions
            if stale:
                first_msg_logged -= stale

        # 清理 last_message_times 中过期的条目
        last_msg_times = getattr(self, "last_message_times", None)
        if last_msg_times and isinstance(last_msg_times, dict):
            stale_keys = [
                k for k, v in last_msg_times.items()
                if current_time - v > 86400
            ]
            for k in stale_keys:
                del last_msg_times[k]
